...
def fetch_xiaoyuzhou_podcasts(max_items=5):
    """Fetch podcasts from 小宇宙 (Xiaoyuzhou FM) via search."""
    podcasts = []
    
    # 用 Exa 搜索卡兹克相关播客
    try:
        result = subprocess.run(
            ['mcporter', 'call', 'exa.web_search_exa(query: "数字生命卡兹克 小宇宙 播客", numResults: 3)'],
...
def main():
    ...
    print("\n🎙️ 卡兹克（小宇宙播客）...")
    try:
        articles = fetch_xiaoyuzhou_podcasts(max_items=3)
